...
const SheetTrigger = React.forwardRef<HTMLButtonElement, SheetTriggerProps>(({ children, asChild, ...props }, forwardedRef) => {
   ...
   return (
      <button ref={forwardedRef} type="button" onClick={() => setOpen(true)} {...props}>
         {children}
      </button>
   );
});
...
const SheetClose = React.forwardRef<HTMLButtonElement, React.ButtonHTMLAttributes<HTMLButtonElement>>(({ children, ...props }, ref) => {
   const { setOpen } = React.useContext(SheetContext) || { setOpen: () => {} };
